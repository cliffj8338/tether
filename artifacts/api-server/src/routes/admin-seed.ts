...
import { refreshDemoTimestampsIfStale } from "../lib/demo-refresh";
import crypto from "crypto";

const router: IRouter = Router();
router.use("/admin/ops/seed-demo", requireAdmin, blockShowcaseWrites);

// Manually roll stale demo timestamps forward to "now" (also runs
// automatically at boot and every 6h — see lib/demo-refresh.ts).
router.post("/admin/ops/seed-demo/refresh", async (req, res) => {
  try {
    const result = await refreshDemoTimestampsIfStale();
    res.json(result);
  } catch (err: any) {
    req.log.error(err, "Demo refresh failed");
